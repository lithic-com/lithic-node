...
describe('resource tokenizations', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.tokenizations.list({
  ...
  test('resendActivationCode: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.tokenizations.resendActivationCode('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { activation_method_type: 'TEXT_TO_CARDHOLDER_NUMBER' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Lithic.NotFoundError);
  });
  ...
  test('updateDigitalCardArt: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.tokenizations.updateDigitalCardArt('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { digital_card_art_token: '5e9483eb-8103-4e16-9794-2106111b2eca' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Lithic.NotFoundError);
  });
});
