...
import { APIPromise } from 'lithic/core/api-promise';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['LITHIC_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Lithic) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    const client = new Lithic({ logger: logger, apiKey: 'My Lithic API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Lithic({ logger: logger, apiKey: 'My Lithic API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'LITHIC_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Lithic({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My Lithic API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Lithic({ apiKey: 'My Lithic API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['LITHIC_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Lithic({ apiKey: 'My Lithic API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Lithic({ apiKey: 'My Lithic API Key', fetch: testFetch });
