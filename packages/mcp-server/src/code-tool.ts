...
const remoteStainlessHandler = async (
  ...
  LITHIC_API_KEY: requireValue(readEnv('LITHIC_API_KEY') ?? client.apiKey, 'set LITHIC_API_KEY environment variable or provide apiKey client option'),
  LITHIC_WEBHOOK_SECRET: readEnv('LITHIC_WEBHOOK_SECRET') ?? client.webhookSecret ?? undefined,
  LITHIC_BASE_URL: readEnv('LITHIC_BASE_URL') ?? readEnv('LITHIC_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
};
